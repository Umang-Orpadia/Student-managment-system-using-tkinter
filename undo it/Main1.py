...
def add_students():
    if Roll_no.get() == "" or name.get() == "":
        ...
        con=pymysql.connect(host="localhost",user="root",password="",database="sms1")
        cur=con.cursor()
        cur.execute("insert into data values(%s,%s,%s,%s,%s,%s,%s)",(Roll_no.get(),name.get(),email.get(),gender.get(),contact.get(),dob.get(),txt_address.get(1.0,tk.END)))
        con.commit()
        fetch_data()
        clear()
        con.close()
...
def update_data():
    ...
    con=pymysql.connect(host="localhost",user="root",password="",database="sms1")
    cur=con.cursor()
    cur.execute("update data set name=%s,email=%s,gender=%s,contact=%s,dob=%s,address=%s where roll_no=%s",(name.get(),email.get(),gender.get(),contact.get(),dob.get(),txt_address.get(1.0,tk.END),Roll_no.get()))
    con.commit()
    fetch_data()
    clear()
    con.close()
...
def delete_data():
    ...
    con=pymysql.connect(host="localhost",user="root",password="",database="sms1")
    cur=con.cursor()
    cur.execute("delete from data where roll_no=%s",Roll_no.get())
    con.commit()
    fetch_data()
...
def search_data():
    con=pymysql.connect(host="localhost",user="root",password="",database="sms1")
    cur=con.cursor()
    cur.execute("select * from data where "+str(search_by.get())+" like '%"+str(search_text.get())+"%'")
    rows=cur.fetchall()
    if len(rows)!=0:
        student_table.delete(*student_table.get_children())
        for row in rows:
